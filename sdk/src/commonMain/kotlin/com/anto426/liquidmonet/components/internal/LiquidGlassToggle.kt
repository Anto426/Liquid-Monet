...
internal fun LiquidGlassToggle(
    ...
    val dampedDragAnimation = remember(animationScope, performance) {
        DampedDragAnimation(
    ...
    LaunchedEffect(enabled, dampedDragAnimation) {
        if (!enabled) {
            fraction = if (currentChecked) 1f else 0f
            didDrag = false
    ...
    // Fade the resting thumb out completely on press so the magnified track remains visible.
        ...
        Box(
            modifier = Modifier
                ...
                .liquidGlassDynamic(
                    ...
                    shape = Capsule(),
                    preset = LiquidGlassDynamicPresets.ToggleThumb,
                    performance = performance,
                        ...
                        drawRect(
                            thumbColor.copy(
                                alpha = thumbColor.alpha *
                                    (1f - dampedDragAnimation.pressProgress.coerceIn(0f, 1f))
                            )
                        )
                    }
                )
