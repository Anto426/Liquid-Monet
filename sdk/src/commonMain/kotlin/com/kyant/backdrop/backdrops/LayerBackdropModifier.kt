...
private class LayerBackdropNode(
    ...
    override fun ContentDrawScope.draw() {
        if (size.width <= 0f || size.height <= 0f || backdrop.graphicsLayer.isReleased) {
            drawContent()
            return
        }
        if (backdrop.recordsContentOnly) {
            // Record the display list once, then replay it for the visible scene. Drawing the
            // entire subtree both before and during recording multiplies work at every nested
            // scene/overlay layer. Custom recording callbacks retain their separate semantics.
