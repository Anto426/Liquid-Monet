...
/**
 * Immutable data payload for AntoToast.
 */
@Immutable
data class AntoToastData(
    val id: Long,
    val message: String,
    val subtitle: String? = null,
    val type: AntoToastType = AntoToastType.Info,
 ...
 */
@Stable
class AntoToastState(private val scope: CoroutineScope) {
    var currentToast by mutableStateOf<AntoToastData?>(null)
        private set
    ...
    fun show(
        ...
        type: AntoToastType = AntoToastType.Info,
        icon: ImageVector? = null,
        durationMillis: Long = 2800L
    ) {
        dismissJob?.cancel()
        val data = AntoToastData(
            id = ++nextId,
            message = message,
            subtitle = subtitle,
            type = type,
...
/**
 * Remembers an [AntoToastState] tied to the current composition scope.
 */
@Composable
fun rememberAntoToastState(): AntoToastState {
    val scope = rememberCoroutineScope()
    return remember(scope) { AntoToastState(scope) }
}

/**
 * AntoToastHost - Overlay container that renders active [AntoToast] notifications with camera punch-hole dewdrop physics.
 */
@Composable
fun AntoToastHost(
    state: AntoToastState,
...
/**
 * AntoToast - Radiant Optical Liquid Glass Toast Notification Pill.
...
fun AntoToast(
    ...
    val toastHighlight = rememberAntoControlHighlight()
    ...
    val accentColor = when (data.type) {
        AntoToastType.Success -> Color(0xFF10B981)
        AntoToastType.Info -> colorScheme.primary
        AntoToastType.Warning -> Color(0xFFF59E0B)
        AntoToastType.Error -> Color(0xFFEF4444)
        AntoToastType.Neutral -> Color.White.copy(alpha = 0.85f)
    }

    val iconVector: ImageVector = data.icon ?: when (data.type) {
        AntoToastType.Success -> AntoIcons.Check
        AntoToastType.Info -> AntoIcons.Info
        AntoToastType.Warning -> AntoIcons.Warning
        AntoToastType.Error -> AntoIcons.Close
        AntoToastType.Neutral -> AntoIcons.Star
    }
    ...
    Box(
        modifier = modifier
            ...
            .antoLiquidGlass(
                ...
                layerBlock = antoControlLayerBlock(true, toastHighlight)
            )
