 ...
 * Call [start] when the state becomes visible and [close] when its lifecycle ends. `AntoUITheme`
 * does this automatically.
 */
@Stable
class AntoGlassPerformanceManager(
    context: Context,
    ...
    val state: State<AntoGlassPerformanceState>
        get() = mutableState
    ...
    private fun readDeviceProfile(): AntoGlassDeviceProfile {
        val memoryInfo = ActivityManager.MemoryInfo()
        activityManager?.getMemoryInfo(memoryInfo)
        return AntoGlassDeviceProfile(
    ...
    private fun readState(): AntoGlassPerformanceState {
        val memoryInfo = ActivityManager.MemoryInfo()
        activityManager?.getMemoryInfo(memoryInfo)
        ...
        val thermalStatus = thermalMonitor?.currentStatus() ?: AntoGlassThermalStatus.UNKNOWN
        ...
        return AntoGlassPerformanceState(
            device = deviceProfile,
            ...
            motionScale = baseScales.motion * intensity,
    ...
    private companion object {
        ...
        private fun selectQualityTier(
            ...
            thermalStatus: AntoGlassThermalStatus
        ): AntoGlassQualityTier {
            var tier = when {
                !device.supportsRenderEffect ||
                    device.isLowRamDevice ||
                    device.cpuCoreCount <= 2 ||
                    device.totalMemoryBytes in 1L until 2L * GIB -> AntoGlassQualityTier.MINIMAL

                device.supportsRuntimeShader &&
                    device.cpuCoreCount >= 8 &&
                    device.totalMemoryBytes >= 8 * GIB -> AntoGlassQualityTier.ULTRA

                device.supportsRuntimeShader &&
                    device.cpuCoreCount >= 4 &&
                    device.totalMemoryBytes >= 4 * GIB -> AntoGlassQualityTier.HIGH

                else -> AntoGlassQualityTier.BALANCED
            }

            if (powerSaveMode) tier = minTier(tier, AntoGlassQualityTier.BALANCED)
            if (memoryPressureHigh) tier = AntoGlassQualityTier.MINIMAL
            tier = when (thermalStatus) {
                AntoGlassThermalStatus.LIGHT -> minTier(tier, AntoGlassQualityTier.HIGH)
                AntoGlassThermalStatus.MODERATE -> minTier(tier, AntoGlassQualityTier.BALANCED)
                AntoGlassThermalStatus.SEVERE,
                AntoGlassThermalStatus.CRITICAL,
                AntoGlassThermalStatus.EMERGENCY,
                AntoGlassThermalStatus.SHUTDOWN -> AntoGlassQualityTier.MINIMAL

                AntoGlassThermalStatus.UNKNOWN,
                AntoGlassThermalStatus.NONE -> tier
            }
            return tier
        }

        private fun minTier(
            first: AntoGlassQualityTier,
            maximum: AntoGlassQualityTier
        ): AntoGlassQualityTier = AntoGlassQualityTier.entries[min(first.ordinal, maximum.ordinal)]

        private fun scalesFor(tier: AntoGlassQualityTier): PerformanceScales = when (tier) {
            AntoGlassQualityTier.MINIMAL -> PerformanceScales(
                blur = 0.4f,
                refraction = 0.2f,
                motion = 0.5f,
            ...
            AntoGlassQualityTier.BALANCED -> PerformanceScales(
                blur = 0.7f,
                refraction = 0.55f,
                motion = 0.75f,
            ...
            AntoGlassQualityTier.HIGH -> PerformanceScales(
                blur = 0.9f,
                refraction = 0.85f,
                motion = 0.9f,
            ...
            AntoGlassQualityTier.ULTRA -> PerformanceScales(
                blur = 1f,
                refraction = 1f,
                motion = 1f,
...
private interface ThermalMonitor {
    ...
    fun currentStatus(): AntoGlassThermalStatus
    fun close()
}
...
private class ThermalMonitorApi29(
    ...
    override fun currentStatus(): AntoGlassThermalStatus = when (powerManager.currentThermalStatus) {
        PowerManager.THERMAL_STATUS_NONE -> AntoGlassThermalStatus.NONE
        PowerManager.THERMAL_STATUS_LIGHT -> AntoGlassThermalStatus.LIGHT
        PowerManager.THERMAL_STATUS_MODERATE -> AntoGlassThermalStatus.MODERATE
        PowerManager.THERMAL_STATUS_SEVERE -> AntoGlassThermalStatus.SEVERE
        PowerManager.THERMAL_STATUS_CRITICAL -> AntoGlassThermalStatus.CRITICAL
        PowerManager.THERMAL_STATUS_EMERGENCY -> AntoGlassThermalStatus.EMERGENCY
        PowerManager.THERMAL_STATUS_SHUTDOWN -> AntoGlassThermalStatus.SHUTDOWN
        else -> AntoGlassThermalStatus.UNKNOWN
    }
